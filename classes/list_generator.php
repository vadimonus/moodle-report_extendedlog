...
/**
 * Class for returning system components and events lists.
 *
 * @package    report_extendedlog
 * @copyright  2016 Vadim Dvorovenko <Vadimon@mail.ru>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class list_generator {
    ...
    /** @var list_generator The singleton instance */
    protected static $singletoninstance;

    /** array of plugin types objects */
    protected $types;
    ...
    /** array List of plugins events */
    protected $pluginevents;

    /** array List of core events */
     ...
     *
     * @return list_generator The singleton instance
     */
    ...
    /**
     * Traverse plugins folders and saves information about plugintypes, plugins and events.
     */
    private function init() {
        ...
        foreach ($types as $type => $directory) {
            $typeinfo = new \stdClass();
            $typeinfo->name = $type;
            $typeinfo->displayname = $pluginman->plugintype_name($type);
            $typeinfo->displaynameplural = $pluginman->plugintype_name_plural($type);
            $this->types[$type] = $typeinfo;
        }

        // Getting plugins.
        foreach ($this->types as $typeinfo) {
            $plugins = $pluginman->get_plugins_of_type($typeinfo->name);
        ...
        foreach ($types as $type => $typedirectory) {
            ...
            foreach ($plugins as $plugin => $plugindirectory) {
                ...
                foreach ($events as $event => $eventdirectory) {
                    ...
                    $plugininfo->typename = $typeinfo->name;
                    $plugininfo->typedisplaynameplural = $typeinfo->displaynameplural;
                    $this->plugins[$plugininfo->name] = $plugininfo;

                    $eventinfo = new \stdClass();
                    $eventname = '\\' . $type . '_' . $plugin . '\\event\\' . $event;
                    $eventinfo->name = $eventname;
                    $eventinfo->displayname = $this->get_event_name($eventname);
                    $eventinfo->typedisplayname = $typeinfo->displayname;
                    $eventinfo->pluginname = $plugininfo->name;
                    $eventinfo->plugindisplayname = $plugininfo->displayname;
                    $this->pluginevents[$eventinfo->name] = $eventinfo;
                }
            }
        }
    ...
    /**
     * Returns a localized name. Supresses warnings to hide warnings on deprecated events.
     *
    ...
    public function get_components_menu() {
        ...
        foreach ($this->plugins as $plugin) {
            ...
            $components[$groupname][$plugin->name] = $displayname;
        }
     ...
     * @return array list of components.
     */
    public function get_events_menu() {
        if (empty($this->coreevents) || empty($this->pluginevents)) {
            $this->init();
        }
        $pluginevents = array();
        foreach ($this->pluginevents as $event) {
            ...
            $pluginevents[$groupname][$event->name] = $displayname;
        }
        foreach ($pluginevents as $key => $events) {
            \core_collator::asort($pluginevents[$key]);
        }
        \core_collator::ksort($pluginevents);
        $topcomponents = array(
            get_string('filter_event_all', 'report_extendedlog') =>
                array('0' => get_string('filter_event_all', 'report_extendedlog')),
            get_string('filter_event_core', 'report_extendedlog') =>
                array('core' => get_string('filter_event_core', 'report_extendedlog')));
        $pluginevents = array_merge($topcomponents, $pluginevents);
        return $pluginevents;
    }
